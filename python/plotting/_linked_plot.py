...
class LinkedPlotWidget(pg.GraphicsLayoutWidget):
    ...
    def add_plot(self, inputs=[], plot_title=""):
        """
        Create a new linked plot.
        Every time this method is called, another linked plot will appear.

        The linked plot may contain a single data series,
        or it may contain several data series which share the same X and Y axes.
        
        All plots will share the same X-axis, subplot or linked plots.
        
        Args:
           inputs(list):
              This is an interable where each element represents a subplot.

              The inputs list must be on the form:
                 [(numpy_array1, data_column_name1, display_name),
                  (numpy_array2, data_column_name2, display_name),
                  ... ]
       
              It is assumed that numpy_array['date'] exists, it will be used as X-axis.
              The numpy_array['date'] column must be either dtype:('date', 'f8')
              as an epoch timestamp as generated by datetime.datetime.timestamp(),
              or it may be a datetime.date object stored in the numpy array with dtype:('date', 'O').

              The data_column_name is the column in numpy_array which contains the Y-axis data.

              The display_name is what the legend will show for this data series.

           plot_title(str): Title of this plot. 
        """
        ...
        # sanity check
        if len(inputs) == 0:
            raise ValueError("The 'inputs' list contains no data")

        # create all plots
        for numpy_array, data_column_name, display_name in inputs:

            # if this is the first subplot in this plot window
            if prev_plot is None:
                pl = self.addPlot(row=self.plot_row_counter,
                                  col=0,
                                  title=plot_title,
                                  y=numpy_array[data_column_name],
                                  x=date_array['date'])
                pl.name = plot_title + "master"
            else:
                subplot = prev_plot.plot(row=self.plot_row_counter,
                                         col=0,
                                         pen=nice_colors.pop(),
                                         y=numpy_array[data_column_name],
                                         x=date_array['date'])
                subplot.name = plot_title + "sub"
        ...
        if mouse_is_over_a_plot:
            for pl in self.plots.values():
                ...
                # if the x position is within the plot
                    ...
                    text = "<span style='color: %s'>%s=%s</span>" % (color, pl.name, str(round(y, 4)))
                    print(pl.name)
